            ...
            for client in self.gmail_clients:
                ...
                try:
                    ...
                    if len(messages) > limit:
                        logging.warning(f"Found {len(messages)} unread messages for {client.email_address}, but limiting to {limit} per cycle.")
    ...
    def run_forever(self, interval: int = 60, start_dashboard: Optional[bool] = None) -> None:
        ...
        while True:
            try:
                ...
                self.db.cleanup_old_data(days=config.DB_RETENTION_DAYS)
                self.run_once()
            except Exception as e:
